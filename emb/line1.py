...
class _LINE(object):
	...
	def __init__(self, graph, rep_size=128, batch_size=1000, negative_ratio=5, order=3):
		...
		self.order = order
		self.g = graph
		self.node_size = len(graph.nodes())
	...
	def batch_iter(self):
		...
		edges = [(look_up[x[0]], look_up[x[1]]) for x in self.g.edges()]

		data_size = len(list(self.g.nodes()))
	...
	def gen_sampling_table(self):
		...
		for edge in self.g.edges():
			node_degree[look_up[edge[0]]] += self.g[edge[0]][edge[1]]["weight"]
		...
		total_sum = sum([self.g[edge[0]][edge[1]]["weight"]
						 for edge in self.g.edges()])
		norm_prob = [self.g[edge[0]][edge[1]]["weight"] *
					 data_size/total_sum for edge in self.g.edges()]
		num_small_block = 0
		num_large_block = 0
		cur_small_block = 0
		cur_large_block = 0
